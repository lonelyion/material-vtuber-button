<template>
  <v-layout column justify-center align-center app>
    <v-flex xs12 sm8 md6>
      <v-card v-if="lives">
        <v-card-title>
          <v-icon class="primary--text" style="margin-right: 8px">mdi-clock-outline</v-icon>直播动态
        </v-card-title>
        <v-card-text>
      ...
      <v-card>
        ...
        <v-card-text>
          <div>
            <div>
              <v-btn
                class="accent ma-1 pa-1"
                @click="get_random_voice()"
              >{{ $t('control.pick_one') }}</v-btn>
              <v-btn class="accent ma-1 pa-1" @click="stop_all()">{{ $t('control.stop') }}</v-btn>
      ...
      <v-card v-for="group in groups" :key="group.name">
        <v-card-title class="headline">{{ group.group_description[$i18n.locale] }}</v-card-title>
        <v-card-text>
          <v-btn
            ...
            height="max-content"
            min-height="36px"
          >{{ item.description[$i18n.locale] }}</v-btn>
        </v-card-text>
      </v-card>
...
export default {
  ...
  methods: {
    ...
    get_random_voice() {
      ...
      this.play(
        random_list.voice_list[
          this.get_random_int(random_list.voice_list.length)
        ]
      )
    },
    stop_all() {
